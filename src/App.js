...
class App extends Component {
  ...
  updateCountriesList = visitedCountryId => {
    const {countriesList} = this.state
    const countryItem = countriesList.find(
      coutry => coutry.id === visitedCountryId,
    )
    console.log(countryItem)
    const updatedCountriesList = countriesList.map(country => {
      if (country.id === visitedCountryId) {
        return {
          ...country,
          isVisited: true,
        }
  ...
  updateVisitedCountriesList = notVisitedCountryId => {
    const {countriesList} = this.state
    const countryItem = countriesList.find(
      coutry => coutry.id === notVisitedCountryId,
    )
    console.log(countryItem)
    const updatedCountriesList = countriesList.map(country => {
      if (country.id === notVisitedCountryId) {
        return {
          ...country,
          isVisited: false,
        }
    ...
    return (
      <CountriesListContext.Provider
        value={{
          ...
          updateVisitedCountriesList: this.updateVisitedCountriesList,
        }}
      >
